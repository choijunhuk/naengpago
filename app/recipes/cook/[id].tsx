  ...
  return (
    <AppScreen scroll={false} testID="cook-screen">
      <View className="min-h-full justify-between gap-6 py-5">
        <View className="flex-row items-center justify-between"><Badge label={`${step + 1} / ${recipe.steps.length}`} tone="success" /><Text className="font-sans text-sm font-semibold text-muted-light dark:text-muted-dark">{recipe.name}</Text><Timer color={iconColors.muted} size={20} strokeWidth={1.5} /></View>
        ...
        <View className="gap-3"><View className="flex-row gap-2"><Button label="이전" variant="secondary" icon={<ChevronLeft color={iconColors.muted} size={20} />} disabled={step === 0} onPress={() => setStep((value) => Math.max(0, value - 1))} /><Button label={isLast ? '요리 완료' : '다음'} icon={isLast ? undefined : <ChevronRight color={iconColors.onPrimary} size={20} />} onPress={() => isLast ? router.replace(`/recipes/complete/${recipe.id}`) : setStep((value) => value + 1)} testID={isLast ? 'cook-complete' : undefined} /></View><Button label="나중에 이어하기" variant="ghost" onPress={() => router.back()} /></View>
      </View>
    </AppScreen>
  );
}
