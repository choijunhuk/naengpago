...
import { useInventoryItem, useInventoryMutations } from '../../src/features/data/inventory';
...
export default function InventoryDetailScreen() {
  ...
  const { updateInventory: update, consumeInventory: consume, deleteInventory: remove } = useInventoryMutations();
  if (itemQuery.isLoading) return <AppScreen title="식재료 상세"><StateView state="loading"><View /></StateView></AppScreen>;
  ...
  return (
    <AppScreen title={item.displayName} subtitle={`${location?.name ?? '위치 미지정'} · ${item.registeredVia === 'IMAGE_AI' ? '사진 AI 등록' : '직접 등록'}`} testID="inventory-detail-screen">
      <Card className="items-center gap-4 py-7">
        <Text className="text-6xl" accessibilityElementsHidden importantForAccessibility="no-hide-descendants">{categoryEmoji(item.category)}</Text>
      ...
      <Card className="gap-4">
        <QuantityEditor quantityType={item.quantityType} quantity={item.quantity} unit={item.unit} remainingLevel={item.remainingLevel} onQuantityChange={(quantity) => void update(item.id, { quantity })} onLevelChange={(remainingLevel) => void update(item.id, { remainingLevel })} />
        <Button label="한 번 사용했어요" onPress={() => { void consume(item); Alert.alert('재고에 반영했어요', '변경 이력이 기록될 준비가 되어 있어요.'); }} />
      </Card>
      ...
      <Button label="재료 삭제" variant="danger" onPress={() => Alert.alert('재료를 삭제할까요?', '재고 목록에서는 사라지지만 실제 서버에서는 이력을 위해 soft delete해요.', [{ text: '취소' }, { text: '삭제', style: 'destructive', onPress: () => { void remove(item.id); router.replace('/(tabs)/inventory'); } }])} />
    </AppScreen>
  );
}
